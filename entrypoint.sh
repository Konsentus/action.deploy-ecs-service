...
aws_account_id=$(cat ${INPUT_ENVIRONMENT_CONFIGURATION} | jq -r '.default.awsAccountId | select(. != null)' | cat ${INPUT_ENVIRONMENT_CONFIGURATION} | jq -r '.default.awsAccountId | select(. != null)')
cluster_name=$(cat ${INPUT_ENVIRONMENT_CONFIGURATION} | jq -r '.default.clusterName | select(. != null)' | cat ${INPUT_ENVIRONMENT_CONFIGURATION} | jq -r '.default.clusterName | select(. != null)')
service_name=$(cat ${INPUT_ENVIRONMENT_CONFIGURATION} | jq -r '.default.serviceName | select(. != null)' | cat ${INPUT_ENVIRONMENT_CONFIGURATION} | jq -r '.default.serviceName | select(. != null)')
