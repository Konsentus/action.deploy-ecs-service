...
aws_account_id=$(cat ${INPUT_ENVIRONMENT_CONFIGURATION} | jq -r .${branch_name}.awsAccountId) || $(cat ${INPUT_ENVIRONMENT_CONFIGURATION} | jq -r .default.awsAccountId)
cluster_name=$(cat ${INPUT_ENVIRONMENT_CONFIGURATION} | jq -r .${branch_name}.clusterName) || $(cat ${INPUT_ENVIRONMENT_CONFIGURATION} | jq -r .default.clusterName)
service_name=$(cat ${INPUT_ENVIRONMENT_CONFIGURATION} | jq -r .${branch_name}.serviceName) || $(cat ${INPUT_ENVIRONMENT_CONFIGURATION} | jq -r .default.serviceName)
